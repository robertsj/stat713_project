...
```{r tidy=TRUE,message=FALSE,warning=FALSE, fig.height=10, fig.width=8}
require(gridExtra)

models = list()
# Make DMD models and predictions
for (r in c(5, 7, -1)){
  tmp <- lm_with_dmd(df, rank=r)
  tmp$pred <- predict_with_dmd(df, tmp$model, tmp$dmd)
  tmp$desc <- sprintf("DMD, rank=%i", tmp$dmd$rank)
  models[[length(models)+1]] <- tmp
}
# Make linear
tmp  <- list(model=lm(z ~ (t+lat+lon), data=df))
tmp$pred <- predict(tmp$model, interval="prediction")
tmp$desc <- sprintf("z ~ (t+lat+lon)")
models[[length(models)+1]] <- tmp
# Make bi-linear polynomial model and prediction
tmp  <- list(model=lm(z ~ (t+lat+lon)^2, data=df))
tmp$pred <- predict(tmp$model, interval="prediction")
tmp$desc <- sprintf("z ~ (t+lat+lon)^2")
models[[length(models)+1]] <- tmp
# Make bi-quadratic polynomial model and prediction
tmp  <- list(model=lm(z ~ (t+lat+lon)^3, data=df))
tmp$pred <- predict(tmp$model, interval="prediction")
tmp$desc <- sprintf("z ~ (t+lat+lon)^3")
models[[length(models)+1]] <- tmp
...
for (i in 1:length(models)){
  ...
  heat$z <- m$pred[, 1] - df$z
  p[[length(p) + 1]] <- ggplot(heat, aes(location, time, fill=z)) + 
                        geom_tile() + scale_fill_distiller(palette = "Spectral", limits=c(-10, 20)) + theme_void() +
                        labs(title=desc) 
}

grid.arrange(grobs=p, ncol=2,padding=20)
```

From these results, it appears the DMD-based models are not competing with the simpler polynomial models based on AIC.  Before moving on, it's worth examining the impact of DMD rank on AIC.  As noted, the "optimal" rank is based on an optimally truncated SVD that is itself based on statistical considerations.  Here, AIC is computed for DMD as a function of rank.  Note, complex-conjugate pairs are not reduced to one basis vector this time, though `lm` does in all cases eliminate the singularities.

```{r}
aic <- zer
ranks <- list()
for (r in 1:31){
  m <- lm_with_dmd(df, rank=r, discard_pairs=FALSE)
  ranks[[r]] <- m$dmd$rank
  aic[[r]] <- AIC(m$model)
}
plot(ranks, aic,xlab="DMD rank", ylab="AIC")
```
